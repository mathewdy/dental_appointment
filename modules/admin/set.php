...
if(isset($_POST['save'])){
  ...
  if(mysqli_num_rows($run_appointment_time) > 0){
    echo "<script> error('Appointment time already booked!', () => window.location.href = 'set-schedule.php?user_id_dentist=$user_id_dentist&user_id_patient=$user_id_patient') </script>";
  }else{
      ...
      if(mysqli_num_rows($run_check_appointment) > 0){
        echo "<script> error('Patient already have an Appointment.', () => window.location.href = 'set-schedule.php?user_id_dentist=$user_id_dentist&user_id_patient=$user_id_patient') </script>";
      }else{
        ...
        if($run_appointment) {
          ...
          echo "<script> error('Something went wrong!', () => window.location.href = 'set-schedule.php?user_id_dentist=$user_id_dentist&user_id_patient=$user_id_patient') </script>";
        }
      }
  }
}
